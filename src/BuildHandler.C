...
void Build::mousePressEvent(QMouseEvent* e) 
{
   ...
   m_deleteTarget  = 0;
   m_molecule      = 0;
   m_buildIsActive = false;

   m_viewer->clearSelection();
   m_viewer->m_buildObjects.clear();
...
void Build::leftMousePressEvent(QMouseEvent* e) 
{
   m_viewer->QGLViewer::select(e);
   // Check to see if we have selected the first atom
   ...
   if ((m_beginAtom == 0) && (nObjects > 0) && (e->modifiers() != int(Viewer::s_buildModifier))) {
   ...
   if (m_beginAtom == 0) {
      ...
      m_beginAtom = m_molecule->createAtom(m_viewer->m_buildElement, v);
      m_viewer->m_buildObjects.append(m_beginAtom);
   }

   m_buildIsActive = true;
...
void Build::rightMousePressEvent(QMouseEvent* e) 
{
   m_viewer->clearSelection();
   m_viewer->QGLViewer::select(e);
   if (m_viewer->m_selectedObjects.size() == 0) return;
...
void Build::leftMouseMoveEvent(QMouseEvent* e) 
{
   ...
   if (shift < 0.2) {
      ...
      if (m_endAtom) {
         m_viewer->m_buildObjects.removeAll(m_endAtom);
         delete m_endAtom;
         m_endAtom = 0;
      }

      if (m_bond) {
         m_viewer->m_buildObjects.removeAll(m_bond);
         delete m_bond;
         m_bond = 0;
      }
      ...
      if (m_existingAtom != m_beginAtom) {
         if (!m_molecule->getBond(m_existingAtom, m_beginAtom)) {
            ...
            if (!molecules.isEmpty() && molecules.first() == m_molecule) {
               ...
               m_viewer->m_buildObjects.append(m_bond);
            }
         }
      }
      ...
      if (!m_endAtom) {
         ...
         m_endAtom = m_molecule->createAtom(m_viewer->m_buildElement, pos);
         m_bond    = m_molecule->createBond(m_beginAtom, m_endAtom);
         m_viewer->m_buildObjects.append(m_endAtom);
         m_viewer->m_buildObjects.append(m_bond);
      }
...
void Build::rightMouseMoveEvent(QMouseEvent* e) 
{
   e->accept();
}
...
void Build::leftMouseReleaseEvent(QMouseEvent* e) 
{
   if (!m_molecule) return;
   Q_UNUSED(e);
   ...
   if (m_bond == 0) {
      if (m_beginAtom != 0 ) {
         if (m_existingAtom == 0 || m_beginAtom == m_existingAtom) {
            ...
            if (m_beginAtom->getAtomicNumber() != (int)m_viewer->m_buildElement) {
               Command::ChangeAtomType* cmd = new Command::ChangeAtomType(m_beginAtom);
               m_beginAtom->setAtomicNumber(m_viewer->m_buildElement);
   ...
   for (int i = 0; i < m_viewer->m_buildObjects.size(); ++i) {
       primitive = qobject_cast<Layer::Primitive*>(m_viewer->m_buildObjects[i]);
   ...
   if (!primitives.isEmpty()) {
      Command::AddPrimitives* cmd = new Command::AddPrimitives(m_molecule, primitives);
      m_viewer->postCommand(cmd);
   }
}
...
void Build::rightMouseReleaseEvent(QMouseEvent* e) 
{
   if (!m_molecule) return;
   m_viewer->clearSelection();
   m_viewer->QGLViewer::select(e);
