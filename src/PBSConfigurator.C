...
void PBSConfigurator::verify()
{
   ...
   PBSQueue* currentQueue(m_queues[m_dialog.queue->currentIndex()]);
   ...
   if (t1 > t2) {
      ...
      return;
   }else if (m_dialog.jobfs->value() > currentQueue->m_maxJobfs) {
      QMsgBox::warning(this, "IQmol", "Jobfs exceeds queue limit of " 
         + QString::number(currentQueue->m_maxJobfs));
      return;
...
void PBSConfigurator::setQueue(PBSQueue* queue)
{
   ...
   m_dialog.jobfs->setRange(queue->m_minJobfs, queue->m_maxJobfs);
   m_dialog.jobfs->setValue(queue->m_defaultJobfs);
