...
namespace AtomicDensity {
   ...
   class Base{
      ...
      public:
         Base(unsigned int atomicNumber) : m_atomicNumber(atomicNumber) { }
         virtual ~Base() { }
         virtual bool isAvailable() const = 0;
         virtual double density(qglviewer::Vec const&) const = 0;
         virtual double gradientNorm(qglviewer::Vec const&) const { return 0.0; }
         unsigned int atomicNumber() const { return m_atomicNumber; }
      ...
      private:
         double binarySearch(double r1, double r2, double y1, double y2, double thresh) const;
   };
   ...
   class AtomShellApproximation : public Base {
      ...
      public:
         AtomShellApproximation(unsigned int Z);
         bool isAvailable() const {
             return (m_atomicNumber <= s_maxAtomicNumber);
         }
         double density(qglviewer::Vec const&) const;

      private:
         static const unsigned int s_maxAtomicNumber;
         static const unsigned int s_expansionLength[];
   ...
   class SuperpositionIonicDensities : public Base {
      ...
      public:
         SuperpositionIonicDensities(unsigned int Z, double const charge = 0.0);
         bool isAvailable() const {
             return (m_atomicNumber <= s_maxAtomicNumber);
         }
         double density(qglviewer::Vec const&) const;
      ...
      private:
         ...
         unsigned int  m_nNeutralData;
         unsigned int  m_nChargedData;
         double m_charge;
   };
   ...
   class VanDerWaals : public Base {
      public:
         VanDerWaals(unsigned int Z, qglviewer::Vec const& center = qglviewer::Vec(0.0,0.0,0.0),
            double const scale = 1.0, double const solventRadius = 0.0);
         bool isAvailable() const { return true; }
         double density(qglviewer::Vec const&) const;
         double computeSignificantRadius(double const) const;
         Layer::Surface::Data surfaceData(int nDivisions, 
            QList<VanDerWaals*> const& atoms = QList<VanDerWaals*>());
         
         
      private:
         Layer::Surface::Data filterSurfaceData(Layer::Surface::Data const& data,
            QList<VanDerWaals*> const& atoms);
         static GLfloat s_vertices[12][3];
         static unsigned int s_indices[20][3];
         void normalize(GLfloat* vec);
         Layer::Surface::Data drawTriangle(GLfloat const* a, GLfloat const* b, 
            GLfloat const* c, int div, bool abEdge = false, bool bcEdge = false, 
            bool caEdge = false);
            
         Layer::Surface::Data chopTriangle(GLfloat const* a, GLfloat const* b, 
            GLfloat const* c, VanDerWaals const* atom, bool);
         double m_radius;
         int m_filterLevel;
         GLfloat m_center[3];
   };
