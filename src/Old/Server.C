...
void Server::setBasicDefaults(Host const host)
{
   ...
   if (host == Local) {
      ...
      m_runFileTemplate = System::TemplateForRunFile();
   }else { 
      // These are specified in the System file as well :(
      m_killCommand     = "/bin/kill -TERM ${JOB_ID}";
      m_queryCommand    = "/bin/ps xc -S -o command=,pid=,time= ${JOB_ID}";
