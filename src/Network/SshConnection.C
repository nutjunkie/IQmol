...
bool SshConnection::checkHost()
{
   ...
   switch (check) {
      ...
      case LIBSSH2_KNOWNHOST_CHECK_MISMATCH:
         m_message = "Host key does not match in\n " + getKnownHostsFile();
         break;

      case LIBSSH2_KNOWNHOST_CHECK_NOTFOUND:
         {
            QString msg("Host not found in\n");
            msg += getKnownHostsFile() + "\n\nAdd host to file?";

            if (QMsgBox::question(QApplication::activeWindow(), "IQmol", msg,
               QMessageBox::Yes | QMessageBox::No) == QMessageBox::No) {
               // User says no
               m_message = "Host key not found in\n" + getKnownHostsFile();
               break;
            }

            // Attempt to add host to known_hosts file
            QString comment("Generated by IQmol");
            int rc = libssh2_knownhost_addc(knownHosts, m_hostname.toLatin1().data(), NULL, 
                fingerprint, length, comment.toLatin1().data(), comment.length(),
                LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW | typemask, NULL);

            if (rc != 0) {
               m_message = "Unable to add host to known_hosts: " + QString::number(rc);
               break;
            }
