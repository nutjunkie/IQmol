...
SshReply::SshReply(SshConnection* connection, unsigned timeout) : 
   m_connection(connection), m_timeout(timeout) 
{ 
...
void SshReply::runSlot()
{
   ...
   try {
      ...
      m_status = TimedOut;
      QLOG_TRACE() << "Network Reply Timeout";
   }catch (Exception& ex) {
      m_status = Error;
      m_message = ex.what();
      QLOG_ERROR() << "Network Reply error:" << m_message;
   }
   finished();
}
...
void SshExecute::run()
{
   ...
   while ( (channel = libssh2_channel_open_session(session)) == 0 &&
      ...
      m_connection->waitSocket(m_timeout);
   }
...
void SshSendFile::run()
{
   QLOG_TRACE() << "SshSendFile:" << m_sourcePath << "->" << m_destinationPath;
...
void SshReceiveFile::run()
{
   QLOG_TRACE() << "SshReceiveFile " << m_destinationPath << "<-" << m_sourcePath;
...
libssh2_trace(session, LIBSSH2_TRACE_SCP);
   QByteArray source(m_sourcePath.toLocal8Bit());
   LIBSSH2_CHANNEL* channel(0);
   struct stat fileInfo;
   ...
   //unsigned fileSize(50812); // in bytes
   ...
   off_t   got(0);
   QString error;
   QLOG_TRACE() <<  "Prepared to receive" << fileSize << "bytes";
