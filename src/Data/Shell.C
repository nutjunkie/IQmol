...
double* Shell::evaluate(Vec const& gridPoint) const
{
   ...
   switch (m_angularMomentum) {
      ...
      case F7:
         ...
         s_values[0] = s * z * (5.0*z*z - 3.0*r2 ) * half ;
         s_values[1] = s * x * (5.0*z*z -     r2 ) * rt3o8;
         s_values[2] = s * y * (5.0*z*z -     r2 ) * rt3o8;
         s_values[3] = s * z * (    x*x -     y*y) * hrt15;
         s_values[4] = s * x*y*z                   * rt15 ;
         s_values[5] = s * x * (    x*x - 3.0*y*y) * rt5o8;
         s_values[6] = s * y * (3.0*x*x -     y*y) * rt5o8;
      break;
