...
NaturalBondOrbitals::NaturalBondOrbitals(
 ...
 : Orbitals(nbos), m_naturalBondOrbitals(nbos)
{
}


QString NaturalBondOrbitals::description(Data::SurfaceInfo const& info, 
   bool const tooltip)
{
   Data::SurfaceType const& type(info.type());

   unsigned index(type.index());
   QString label(m_naturalBondOrbitals.label(index));

   if (tooltip) {
      Data::SurfaceType::Kind kind(type.kind());
      double occupancy(0.0);
      if (kind == Data::SurfaceType::AlphaOrbital) {
         occupancy = m_naturalBondOrbitals.alphaOccupancy(index);
      }else {
         occupancy = m_naturalBondOrbitals.betaOccupancy(index);
      }
