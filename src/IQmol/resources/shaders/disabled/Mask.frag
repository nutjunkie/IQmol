...
{
   return luminance(vec3(texture2D(Normal, coord)));
}


float depth(vec2 coord)
{
   return texture2D(Normal, coord).a;
}


vec3 normal(vec2 coord)
{
   vec3 n = vec3(texture2D(Normal, coord));
   return 2.0*n-vec3(1.0, 1.0, 1.0);
}
...
float alphaMap(vec2 coord) 
{
   float alpha;
   vec3 n = vec3(texture2D(Normal, coord));

   if (length(dFdx(n) + dFdy(n)) < 0.0) {
      alpha = 0.0;
   }else {
      alpha = 1.0 - abs(dot(viewDirection, n));
   }
   return alpha;
}
...
void main()
{
   ...
   gl_FragColor.r = edgeMap(texcoord, Normal_delta.x, Normal_delta.y);
   gl_FragColor.g = ambientOcclusion(texcoord);
   gl_FragColor.b = 1.0;  //depth(texcoord);
