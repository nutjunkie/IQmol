...
ColorGrid::ColorGrid(Grid* grid, Gradient::Function const& gradient)
  : m_grid(grid), m_gradient(gradient)
{
   QPair<double, double> range(grid->range());
   m_min = range.first;
   m_max = range.second;

   // For data that straddles zero, we assume that we want the scale 
   // to go thesame amount in the positive and negative directions.
...
{
   double value((*m_grid)(qglviewer::Vec(x,y,z)));
