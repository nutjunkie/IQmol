...
Molecule::Molecule(Layer::Molecule* molecule) : m_molecule(molecule)
{
   ...
   m_moleculeConfigurator.surfaceCombo->addItem("van der Waals", VanDerWaals);
   m_moleculeConfigurator.surfaceCombo->addItem("Promolecule", Promolecule);
   //m_moleculeConfigurator.surfaceCombo->addItem("SolventExcluded", SolventExcluded);
   m_moleculeConfigurator.surfaceCombo->addItem("SID", SID);
...
{
   int quality(m_moleculeConfigurator.qualitySlider->value());
   ...
   Grid::DataType::Type type(Grid::DataType::Undefined);
   switch (qvar.toUInt()) {
      case VanDerWaals:
        type = Grid::DataType::VanDerWaals;
        break;
      case Promolecule:
        type = Grid::DataType::Promolecule;
        break;
      case SolventExcluded:
        type = Grid::DataType::SolventExcluded;
        break;
      case SID:
        type = Grid::DataType::SID;
        break;
   }
  
   Grid::DataType data(type);
   surfaceRequested(new Layer::Surface(data, quality, isovalue, positive));
}
